...
- ✅ self-hosting fragments — RoseGold front-end pieces written in RoseGold, each **byte-identical to the canonical C++ stage**: a **faithful lexer** (`rglexer.rg` vs `--tokens`), a **recursive-descent parser** (`rgparser.rg` vs `--ast`), and a **type checker** (`rgchecker.rg` vs `--check` — name resolution + call checking), plus a full tokenize→parse→eval pipeline over a recursive enum AST. Lexer → parser → checker are self-hosted; the bytecode compiler is the remaining rung
