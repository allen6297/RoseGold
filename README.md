...
- ✅ embeddable runtime + native FFI — a C++ host registers native functions scripts can call, loads a script, and ticks its functions each frame with persistent state (`cpp/src/runtime.hpp`; demo engine `cpp/embed/engine.cpp` driving `cpp/embed/behavior.rg`). Scripts declare the host's functions and opaque types in an **`extern:` block** (`extern fn` / `extern type`) — type-checked standalone, **link-checked** against the host registry at load, then bound by name (`cpp/embed/externdemo.cpp` + `externdemo.rg`; a missing-native rejection is exercised by `cpp/embed/linkcheck.cpp`)
