...
struct ClassAst { std::string name; int nameLine = 0, nameCol = 1; std::vector<std::string> generics; Bounds bounds; std::string extends; std::vector<std::string> uses; int vis = 0; std::vector<Field> fields; std::vector<SignalDecl> signals; bool hasCtor = false; std::vector<std::string> ctorParams; std::vector<std::pair<int, int>> ctorParamPos; std::vector<TyNodeP> ctorPtypes; std::vector<Stmt> ctorBody; std::vector<Func> methods; };
...
struct Parsed { std::string module; std::vector<Import> imports; std::vector<Func> funcs; std::vector<Stmt> globals; std::vector<Stmt> initBody; bool hasInit = false; std::vector<ClassAst> classes; std::vector<TraitAst> traits; std::vector<ExtendAst> extensions; std::vector<EnumAst> enums; };
